...
当前云文档 provider 为 WPS 365。先执行 `wps365-cli config init` 和 `wps365-cli auth login --device` 完成授权；旧缓存仍可直接复用，强制联网刷新时才需要有效授权。
...
用于重建全量看板。默认优先使用本地缓存，只有正文缓存缺失、过期或目录缓存不存在时才调用 WPS 365 API。
